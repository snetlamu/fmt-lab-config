...
resource "sccfm_ftd_device" "hq_ngfw" {
  name               = var.device_name[0]
  licenses           = ["BASE", "MALWARE", "THREAT", "URLFilter"]
  virtual            = true
  performance_tier   = "FTDv10"
  access_policy_name = fmc_access_control_policy.dcloud-pod-hq-ftdv.name #var.dcloud-pod-hq-ftdv_policy_name
...
resource "null_resource" "hq_onboarding_script" {
  triggers = {
    generated_command = sccfm_ftd_device.hq_ngfw.generated_command
    host_ip           = var.ftd_ips[0]
  }
...
resource "sccfm_ftd_device_onboarding" "hq_onboarding" {
  depends_on = [null_resource.hq_onboarding_script]
  ftd_uid    = sccfm_ftd_device.hq_ngfw.id
}
